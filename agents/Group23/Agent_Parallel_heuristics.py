...
class MCTS:
    ...
    def rollout(self, board, next_player, red_moves, blue_moves):
        ...
        sim_moves = list(board.empty_spots)
        ...
        # Build frontier: empty neighbors of already played stones
        frontier = set()
        for (r, c) in red_moves | blue_moves:
            for k in range(Tile.NEIGHBOUR_COUNT):
                nr = r + Tile.I_DISPLACEMENTS[k]
                nc = c + Tile.J_DISPLACEMENTS[k]
                if (nr, nc) in sim_set:
                    frontier.add((nr, nc))

        EPS = 0.35        # probability of pure random
        FRONTIER_MIN = 6  # avoid over-bias when frontier is tiny

        while sim_moves and board.winner is None:
            use_frontier = (
                frontier and
                len(frontier) >= FRONTIER_MIN and
                rand() > EPS
            )

            if use_frontier:
                move = random.choice(tuple(frontier))
                frontier.remove(move)
                sim_set.remove(move)
                sim_moves.remove(move)
            else:
                idx = randrange(len(sim_moves))
            ...
            # Expand frontier with neighbors of the played move
            r, c = move
            for k in range(Tile.NEIGHBOUR_COUNT):
                nr = r + Tile.I_DISPLACEMENTS[k]
                nc = c + Tile.J_DISPLACEMENTS[k]
...
class Agent(AgentBase):
    ...
    def make_move(self, turn: int, board: Board, opp_move: Move | None) -> Move:
        # We play first, play an opener move
        if turn == 1 and self.colour == Colour.RED:
            r, c = self.opening_move_11x11()
            return Move(r, c)
        
        # We play second, check for pie rule
        if turn == 2 and opp_move is not None and self.colour == Colour.BLUE:
            if self.should_swap_11x11(opp_move):
                return Move(-1, -1) 
    ...
    def should_swap_11x11(self, mv):
        x, y = mv._x, mv._y 
        mid = 5

        # Center or near-center
        if abs(x - mid) + abs(y - mid) <= 2:
            return True

        # Strong A-column openings
        # A4, A5, A7, A8, A10, A11
        if x == 0 and y in (3, 4, 6, 7, 9, 10):
            return True
