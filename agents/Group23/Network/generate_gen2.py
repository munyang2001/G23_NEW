...
# --- CONFIGURATION ---
MODEL_PATH = "agents/Group23/Network/hex_model_gen2.pt"  # Ensure correct path
NUM_GAMES = 1000  # Goal for Gen-1
TIME_LIMIT = 0.4  # Fast simulations (~200-400 sims per move)
TEMP_THRESHOLD = 10  # Number of moves to use temperature exploration
...
def main():
    ...
    for i in tqdm(range(NUM_GAMES)):
        ...
        if (i + 1) % 100 == 0:
            temp_name = f"gen2_checkpoint_{i + 1}.pt"
            torch.save(all_training_samples, temp_name)

    # 4. Final Save
    run_id = uuid.uuid4().hex[:8]
    final_filename = f"gen2_data_FULL_{run_id}.pt"
    ...
    print(f"Saved Gen 2 Data to: {final_filename}")
    print("=" * 40)
